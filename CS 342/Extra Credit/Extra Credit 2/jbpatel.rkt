...
(require "lazyEvaluationTestCases.rkt")
...
(define (findvalue x env heap)
  (if (null? env)  
      ...
      (if (equal? (car (car env)) x)
          (eval (cadr (car env)) (cdr env) heap) ;; problem here
          (findvalue x (cdr env) heap)))) ;;problem here
...
#|
Answer for part2:
I would say yes since the order is changed as compare to hw 4 and 5 that we did and lazy evaulation
only evaluates when needed. For example if we do an example that has a heap say (1 free) (2 free)
the answer would be 3 and not 2. X evaluates twice. So the result also has different semantics.
|#
