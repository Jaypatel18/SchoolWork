...
void npc_delete(npc_t *n)
{
  ...
  npc_characteristics_t g_character(npc_t *a){
    return a->characteristics;}

  void s_character (npc_t *a, npc_characteristics_t ch){
    a->characteristics = ch; }

  uint32_t g_seen(npc_t *a) {
    return a->have_seen_pc; }

  void s_seen(npc_t *a, unint32_t b){
    a->have_seen_pc = b; }

  int8_t g_pcx (npc_t *a) {
    return a->pc_last_known_position[dim_x]; }

  void s_pcx (npc_t *a, int8_t c){
    a->position[dim_x] = c;  }

  int8_t g_pcy (npc_t *a) {
    return a->pc_last_known_position[dim_y]; }

  void s_pcy (npc_t *n, int8_t d){
    n -> position[dim_y] = d; }
...
void gen_monsters(dungeon_t *d)
{
  ...
  for (i = 0; i < d->num_monsters; i++) {
    ...
    s_speed((character *) m,  rand_range(5, 20);
	    s_life ((character *) m,  1);
    m->sequence_number = ++d->character_sequence_number;
 
	    s_character((npc_t *)m, rand() & 0x0000000f);
	    s_symbol((character *)m,  symbol[m->npc->characteristics]);
	    s_seen ((npc_t *)m,0);
...
static void npc_next_pos_00(dungeon_t *d, character_t *c, pair_t next)
{
  ...
  if (can_see(d, c,(character *) d->pc)) {
    s_pcy ((npc_t *)c, y_value((character *) d->pc));
    s_pcx ((npc_t *)c, x_value ((character *) d->pc));
...
static void npc_next_pos_01(dungeon_t *d, character_t *c, pair_t next)
{
  ...
  if (can_see(d, c, (character*))d -> pc) {
    ...
    npc_next_pos_line_of_sight(d, c, next);
  } else if (g_seen((npc_t *c)) {
    npc_next_pos_line_of_sight(d, c, next);
  }

    if ((next[dim_x] == g_pcx((npc_t *)c) &&
	 (next[dim_y] == g_pcy ((npc_t *)c)))) {
      s_seen(npc_t *)c,0);
  }
}
...
static void npc_next_pos_05(dungeon_t *d, character_t *c, pair_t next)
{
  ...
  if (can_see(d, c, &d->pc(character *)d->pc)) {
   s_pcy ((npc_t *)c, y_value((character *) d->pc));
   s_pcx ((npc_t *)c, x_value((character *) d->pc));
   s_seen((npc_t *c),1);
   npc_next_pos_line_of_sight(d, c, next);
  } else if (s_seen((npc_t *)c)) {
