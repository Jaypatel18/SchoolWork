...
int UI::combat()
{
   clear();
   bool a = false;
   if(!dungeon->vnpcv.empty()&& pc->equipment->isEmpty(2)== a)
   {
      mvprintw(21,21,"works");
      selectTarget();
      return -1;
   }
   else
   {
      mvprintw(1, 1, "Error:No Range or No Monster");
      while(1)
      {
         int a = getch();  
         switch(a)
...
{	
	...
	while (1) {
		...
		switch (ch) {
			...
			case 'R':
			case 'r':
a = dungeon->cmap[dungeon->vnpcv[index]->getY()][dungeon->vnpcv[index]->getX()];

attack = a->hp - pc->equipment->slots[2]->dam->roll();
			if(attack <= 0)
			{ a -> hp = 0;
			  a -> setDead();
dungeon->cmap[dungeon->vnpcv[index]->getY()][dungeon->vnpcv[index]->getX()] = NULL; }
   			else
{ a ->hp -= pc->equipment->slots[2]->dam->roll(); }
			return -1;
