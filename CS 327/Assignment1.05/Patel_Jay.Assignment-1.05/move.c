...
{
	pair_t n;
	n[dim_x] = d->pc.position[dim_x];
	n[dim_y] = d->pc.position[dim_y];
	
	switch (dir) 
{
	
	case 0:
	n[dim_x] = n[dim_x] - 1; //upper left
	n[dim_y] = n[dim_y] - 1;
	break;
	
	case 1:
	n[dim_y] = n[dim_y] - 1; //move up
	break;
	
	case 2:
	n[dim_x] = n[dim_x] + 1; //upper right
	n[dim_y] = n[dim_y] - 1;
	break;
	
	case 3:
	n[dim_x] = n[dim_x] + 1; //move right
	break;
	
	case 4:
	n[dim_x] = n[dim_x] + 1; //lower right
	n[dim_y] = n[dim_y] + 1;
	break;
	...
	if(d->hardness[n[dim_y]][n[dim_x]] != 0)
	{
		return 0;
	} 
	else
	{
		move_character(d,&d->pc,n);
		return 1;
	}
