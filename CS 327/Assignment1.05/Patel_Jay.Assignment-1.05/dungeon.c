...
int j;
for(j = 0; j < (d->num_rooms/2 + d->num_rooms%2);j++)
{
	int randx;
	int randy;
	randx = rand_range(1,78);
	randy = rand_range(1,19);

	while (d->map[randy][randx] != ter_floor_room)
{
	randx = rand_range(1,78);
	randy = rand_range(1,19);
}

d->map[randy][randx] = down;
  ...
  for (p[dim_y] = 0; p[dim_y] < DUNGEON_Y; p[dim_y]++) {
    for (p[dim_x] = 0; p[dim_x] < DUNGEON_X; p[dim_x]++) {
      if (d->character[p[dim_y]][p[dim_x]]) {
        addch(d->character[p[dim_y]][p[dim_x]]->symbol);
      } else {
        switch (mappair(p)) {
        case ter_wall:
