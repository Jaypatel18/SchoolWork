...
static void update(Character *c, int x, int y)
{
	...
	{
		Item *j = dungeon -> imap[y][x];
		for(unsigned int i=0;i < dungeon->itemv.size();i++)
		{
			if(dungeon->itemv.at(i) == j)
			dungeon->itemv.erase(dungeon->itemv.begin()+i);
		}
		
			if(pc->inventory.size()<10)
			{
				pc-> inventory.push_back(j);
				dungeon->imap[y][x] = NULL;
			}
	}
}
