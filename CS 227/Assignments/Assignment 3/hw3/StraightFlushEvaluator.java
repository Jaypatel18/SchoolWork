 ...
 *         ranks are consecutive numbers AND the cards all have the same suit.
 *         The number of required cards is equal to the hand size. An ace (card
 *         of rank 1) may be treated as the highest possible card or as the
 *         lowest (not both) To evaluate a straight containing an ace it is
 *         necessary to know what the highest card rank will be in a given game;
 *         therefore, this value must be specified when the evaluator is
 *         constructed. In a hand created by this evaluator the cards are listed
 *         in descending order with high card first, e.g. [10 9 8 7 6] or [A K Q
 *         J 10], with one exception: In case of an ace-low straight, the ace
...
//		 * @return
//		 */
//		return new Hand(cards, sideCards, new StraightFlushEvaluator(
...
//	}
	...
	public Hand getBestHand(Card[] allCards) {
		...
		Hand hands = null;
		IEvaluator eve = new StraightFlushEvaluator(rank, hand, maxCardNum);
