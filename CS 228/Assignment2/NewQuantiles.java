...
public class Quantiles {
	...
	public Quantiles(int[] data, int q, boolean fast) {
		...
		if(fast == true){
			recursion(data,0,q-2);
			else{
				new Quantiles(data,q);
		}
		for(int i = 0 ; i < n; i ++){
	 ...
	 */
		...
		private void recursion(int[] data,int first, int last){
			if(first > last){
				return;
			}
			int mid = (first+last)/2;
			quantiles[mid] = OrderStatistics.findOderStatistics(data,(int)Math.ceil((mid+1)*(n/(float)q)));
			recursion(data,first,mid-1);
			recursion(data,mid+1,last);
		}
