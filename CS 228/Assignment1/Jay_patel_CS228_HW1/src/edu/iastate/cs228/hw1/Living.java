...
public abstract class Living {
	...
	protected void census(int population[]) {
		...
		// respectively.
		Living[][] neighbors = neighbours(row, column);
		for (int i = 0; i < 3; i++)
			for (int j = 0; j < 3; j++) {
				if (neighbors[i][j] != null) {
					int index = getIndex(neighbors[i][j]);
					if (index != -1)
						population[index]++;
				}
			}
	...
	protected int getIndex(Living neighbor) {
		switch (neighbor.who()) {
		case BADGER:
			return 0;
		case EMPTY:
	...
	protected boolean checkWithInBoundry(int p, int q, int width) {
		if ((p < 0) || (q < 0) || (p >= width) || (q >= width))
			return false;
		return true;
	}

	protected Living[][] neighbours(int i, int j) {
		Living[][] neighbors = new Living[3][3];
		int p = -1, q = -1;
		...
		for (int x = 0; x < 3; x++) {
			q = j - 1;
			for (int y = 0; y < 3; y++) {
				if (checkWithInBoundry(p, q, width))
					neighbors[x][y] = world.grid[p][q];
				else
					neighbors[x][y] = null;
				q++;
			}
